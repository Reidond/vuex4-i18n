 ...
 ** file vuex-i18n-defaultStore.js to include a respective vuex module.
 */
...
VuexI18nPlugin.install = function install(app, store, config) {
  ...
  defaultStore.registerModule(moduleName, module, {
  ...
  // get localized string from defaultStore. note that we pass the arguments passed
  ...
  let translateInLanguage = function translateInLanguage(locale) {
    ...
    // check if the language exists in the defaultStore. return the key if not
    if (translations.hasOwnProperty(locale) === false) {
      translationExists = false;

      // check if the key exists in the defaultStore. return the key if not
    ...
    // check if a vaild fallback exists in the defaultStore.
    // return the default value if not
    if (translations.hasOwnProperty(fallback) === false) {
      return render(locale, defaultValue, options, pluralization);
    }

    // check if the key exists in the fallback locale in the defaultStore.
    // return the default value if not
  ...
  let translateInLanguageFilter = function translateInLanguageFilter(
    key,
    locale,
    ...args
  ) {
  ...
  let checkKeyExists = function checkKeyExists(key, scope = "fallback") {
    ...
    if (
      ...
      translations[locale].hasOwnProperty(key)
    ) {
      return true;
    }
    ...
    if (
      translations.hasOwnProperty(fallback) &&
      translations[fallback].hasOwnProperty(key)
    ) {
      return true;
    }
  ...
  let phaseOutExistsFn = function phaseOutExistsFn(locale) {
    if (config.warnings)
      console.warn(
        "i18n: $i18n.exists is depreceated. Please use $i18n.localeExists instead. It provides exactly the same functionality.",
      );
...
let renderFn = function (identifiers, warnings = true) {
  ...
  let matcher = new RegExp(
    "" + identifiers[0] + "{1}(\\w{1}|\\w.+?)" + identifiers[1] + "{1}",
    "g",
  );
  ...
  let replace = function replace(translation, replacements) {
    ...
    return translation.replace(matcher, function (placeholder) {
      ...
      let key = placeholder
        .replace(identifiers[0], "")
        .replace(identifiers[1], "");
